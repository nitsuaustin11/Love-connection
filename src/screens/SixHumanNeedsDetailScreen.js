...
import { formatSixHumanNeedsResult } from '../services/personalityDataService';

const SixHumanNeedsDetailScreen = ({ navigation, route }) => {
  const { user } = useAuthStore();
  const { testData } = route.params;

  const testInfo = testData?.Test_information;
  const uiInfo = testData?.Ui_information || {};
  const userResults = user?.personalityTests?.sixHumanNeeds;
  const resultText = formatSixHumanNeedsResult(userResults, testData);
  ...
  const handleAddResults = () => {
    navigation.navigate('AddSixNeedsResults', { testData });
  };
  ...
  const renderDetailedResults = () => {
    ...
    return (
      <View style={styles.detailedResult}>
        <Text style={styles.resultTitle}>Your Top Human Needs</Text>
        {sortedResults.map(([need, percentage], index) => {
          const needInfo = testInfo.types_of_results?.find(
            result => result.result_name === need
          );

          return (
            <View key={need} style={styles.needItem}>
              <View style={styles.needHeader}>
                <Text style={styles.needRank}>#{index + 1}</Text>
                <Text style={styles.needName}>
                  {need.split('_').map(word =>
                    word.charAt(0).toUpperCase() + word.slice(1)
                  ).join(' ')}
                </Text>
                <Text style={styles.needPercentage}>{percentage}%</Text>
              </View>
              {needInfo && (
                <Text style={styles.needDescription}>{needInfo.result_description}</Text>
              )}
            </View>
          );
        })}
...
export default SixHumanNeedsDetailScreen;